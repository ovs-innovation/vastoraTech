...
const fallbackBg = "/assets/img/blog/blog-details-banner-1.jpg";

const BreadcrumbTen = ({ blog }: BreadcrumbTenProps) => {
  const bg_img = blog?.image || fallbackBg;
  const top_title = blog?.category || "Category";
  const title = blog?.title ? <>{blog.title}</> : <>Blog Title</>;
  const post_writer = blog?.author || "Author";
  const time = blog?.createdAt ? new Date(blog.createdAt).toLocaleDateString() : "";
  const comments = ""; // Fill if available

  return (
    <>
      <section className="blog-details-area blog-details-bg pb-120 pt-200" style={{ backgroundImage: `url(${bg_img})` }}>
